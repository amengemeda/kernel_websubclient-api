...
import io.mosip.kernel.websub.api.constants.CryptoConstants;
...
public class CryptoUtil {
    ...
    public static CryptoConstants Constants = new CryptoConstants();
    public CryptoUtil(){

    }
    public static String sign(String jsonString) throws Exception { //signs the string

        JsonWebSignature jwSign = new JsonWebSignature();
        KeyStore.PrivateKeyEntry keyEntry = loadP12();
    ...
    public static KeyStore.PrivateKeyEntry loadP12() throws Exception {
        KeyStore.PrivateKeyEntry privateKeyEntry = null;
        String cyptoPassword=Constants.p12password;
        try {
            KeyStore keyStore = KeyStore.getInstance("PKCS12");
            InputStream in = new FileInputStream(Constants.p12Path);//getClass().getClassLoader().getResourceAsStream(fileName);
            ...
            privateKeyEntry = (KeyStore.PrivateKeyEntry) keyStore.getEntry(Constants.p12password ,password);
        }catch (Exception e) {
    ...
    public static X509Certificate getCertificate() throws Exception {
        ...
        InputStream inStream = new FileInputStream(Constants.faydaCertPath);
...
//        System.out.println(encodedCipher);
    ...
    public static byte[]  symmetricEncrypt(SecretKey key, byte[] data, byte[] aad) { //aad?
...
/*
    public static String symmetricDecryptResponse(String cipherText) throws Exception {

        byte[][] splitData = splitAtFirstOccurrence(doDecode(cipherText), Constants.getSplitter().getBytes());
        ...
        byte[] decryptedKey = decryptSecretKey(loadP12().getPrivateKey(), splitData[0]);
